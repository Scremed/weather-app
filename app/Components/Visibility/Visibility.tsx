...
function Visibility() {
    ...
    const getVisibilityText = (visibility: number) => {
        ...
        if(visibilityInKm > 10) return 'Excellent: Clear visibility';
        if(visibilityInKm > 5) return 'Good: Clear visibility';
        if(visibilityInKm > 2) return 'Moderate: Hazy visibility';
        if(visibilityInKm <= 2) return 'Poor: Poor visibility';
